...
INSERT INTO Secuencia (NombreSecuencia)
VALUES ('seqFactura');
...
BEGIN
    SELECT EXTRACT(HOUR FROM (FechaSalida - FechaEntrada) DAY TO SECOND) || ' horas ' || EXTRACT(MINUTE FROM (FechaSalida - FechaEntrada) DAY TO SECOND) || ' minutos' AS diferencia
...
BEGIN
    SELECT EXTRACT(HOUR FROM (fechaSalida - fechaEntrada) DAY TO SECOND)
...
BEGIN
    ...
    DBMS_OUTPUT.PUT_LINE('codigoFactura: ' || v_codigoFactura);
    
    DBMS_OUTPUT.PUT_LINE('infoHoras.fechaEntrada: ' || infoHoras.fechaEntrada);
    DBMS_OUTPUT.PUT_LINE('infoHoras.fechaSalida: ' || infoHoras.fechaSalida);
